...
import { YnabBudgetClient, type YnabApiPort } from "../../src/ynab/ynabBudgetClient.js";

describe("YNAB budget client", () => {
  it("reads a category snapshot from the YNAB month category endpoint", async () => {
    const api = apiFixture({
      getMonthCategoryById: vi.fn<YnabApiPort["categories"]["getMonthCategoryById"]>().mockResolvedValue({
        data: {
  ...
  it("updates a category's budgeted amount through the YNAB category endpoint", async () => {
    const updateMonthCategory = vi.fn<YnabApiPort["categories"]["updateMonthCategory"]>().mockResolvedValue({});
  ...
  it("lists YNAB budgets without account details", async () => {
    const getPlans = vi.fn<YnabApiPort["plans"]["getPlans"]>().mockResolvedValue({
      data: {
        plans: [
  ...
  it("lists copyable non-internal, non-deleted categories with hidden categories flagged", async () => {
    const getCategories = vi.fn<YnabApiPort["categories"]["getCategories"]>().mockResolvedValue({
      data: {
        category_groups: [
          {
            id: "group-1",
...
function apiFixture(overrides: {
  readonly getPlans?: YnabApiPort["plans"]["getPlans"];
  readonly getCategories?: YnabApiPort["categories"]["getCategories"];
  readonly getMonthCategoryById?: YnabApiPort["categories"]["getMonthCategoryById"];
  readonly updateMonthCategory?: YnabApiPort["categories"]["updateMonthCategory"];
}): YnabApiPort {
  return {
    plans: {
      getPlans: overrides.getPlans ?? (async () => ({ data: { plans: [] } })),
    },
    categories: {
      getCategories: overrides.getCategories ?? (async () => ({ data: { category_groups: [] } })),
      getMonthCategoryById:
        overrides.getMonthCategoryById ??
        (async () => ({ data: { category: { id: "category-1", budgeted: 0, activity: 0, balance: 0 } } })),
      updateMonthCategory: overrides.updateMonthCategory ?? (async () => undefined),
    },
  };
}
